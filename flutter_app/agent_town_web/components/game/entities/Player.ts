...
export class Player {
  ...
  /** Called by OfficeScene touch handler to set virtual joystick velocity. */
  setTouchVelocity(vx: number, vy: number) {
    this.touchVX = vx;
    this.touchVY = vy;
  }
  ...
  update() {
    ...
    // Use virtual joystick if no keyboard input
    if (vx === 0 && vy === 0 && (this.touchVX !== 0 || this.touchVY !== 0)) {
      vx = this.touchVX;
      vy = this.touchVY;
    }
