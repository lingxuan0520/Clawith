...
export class InteractionMenu {
  ...
  /**
   * Manual hit test for pointer events — bypasses Phaser's interactive object
   * system which is unreliable inside WebView containers with scrollFactor(0).
   *
   * Call this from the scene's pointerdown handler when the menu is visible.
   * Returns true if the pointer was handled (menu item clicked or outside → close).
   */
  handlePointerAt(screenX: number, screenY: number): boolean {
    if (!this._visible) return false;

    // Ignore clicks in the first 2 frames (prevent accidental double-tap)
    const elapsed = this.scene.game.getFrame() - this.openFrame;
    if (elapsed < 2) return true; // consume but don't act
